...
void HashMap_iteri(
    HashMap *map, void (*fn)(const char *, void *, void *, int), void *arg
)
{
  for (int i = 0; i < map->capacity; i++) {
    if (map->elts[i].is_used) {
      fn(map->elts[i].key, map->elts[i].val, arg, i);
    }
  }
}


int HashMap_any(HashMap *map, int (*fn)(const char *, void *, void *), void *arg)
{
  for (int i = 0; i < map->capacity; i++) {
    if (map->elts[i].is_used) {
      if (fn(map->elts[i].key, map->elts[i].val, arg))
        return 1;
    }
  }
  return 0;
...
void header_upper_case_underscored(const char *s, char *out)
{
  size_t pos = 0;
  while (*s) {
    if (*s == '.') {
...
HashMap *si_map_init()
{
  ...
  out = HashMap_construct(sizeof(int));

  return out;
}

void si_map_set(HashMap *map, const char *key, int val)
{
  HashMap_set(map, key, (void*)&val);
}


int si_map_get(HashMap *map, const char *key)
{
  int *out;

  if ((out = (int *)HashMap_get(map, key, NULL)) == NULL) {
    return SI_DEFAULT;
  }
  return *out;
}
...
void si_map_test()
{
  ...
  for (int i = 0; i < 1000; i++) {
    ...
    si_map_set(si_map, str_key, i);
    val = si_map_get(si_map, str_key);
    if (val != i) {
      printf("Failed: Got %d, Want: %d\n", val, i);
    } else {
      passed++;
    }
  }
